...
project.run do
  ...
  add_operator("DUE_DATE", "DUE_DATE(\"*date in format: yyyy-mm-dd*\") - add(set) due date of selected Task.")
  add_operator("EXECUTORS", "EXECUTORS(\"*executors separated by a comma*\") - add(set) list of executors to selected Task.")
  add_operator("ADDITIONAL_DESCRIPTION", "ADDITIONAL_DESCRIPTION(\"*name and value(text) of description, separated with colon ':'*\") - add(set) unique description to selected Task.")
  ...
  add_operator("SAVE_TO_FILE", "SAVE_TO_FILE(\"*file name or path*\") - save all created Tasks of project in text file.")
  add_operator("LOAD_FROM_FILE", "LOAD_FROM_FILE(\"*file name or path*\") - load all Tasks (and their descriptions) of project from some text file.")
